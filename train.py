...
if __name__ == '__main__':
    ...
    leak_gan = LeakGan(gen_embed_dim=cfg.gen_embed_dim,
                       ...
                       device=cfg.device,
                       gen_ckpt_path=cfg.pre_gen_checkpoint_path + '\\gen目标文件',
                       dis_ckpt_path=cfg.pre_dis_checkpoint_path + '\\dis目标文件',
            ...
            for step in range(cfg.d_step):
                ...
                for epoch in range(cfg.d_epoch):
                    ...
                    print('pretrain discriminator loss is {:.5f}'.format(leak_gan.dis_loss / data_gen.get_train_len()),
                          'pretrain discriminator acc is {:.3f}'.format(
                              leak_gan.dis_acc / data_gen.get_train_len() * 100))

                    torch.save({'state_dict': leak_gan.dis.state_dict(),
                                'loss': leak_gan.dis_loss / data_gen.get_train_len()},
                               cfg.pre_dis_checkpoint_path + '\\Epoch{:0>3d}pre_discriminator_loss{:.5f}.pth.tar'.format(
        ...
        for adv_epoch in range(cfg.ADV_train_epoch):
            ...
            for step in range(cfg.d_step):
                ...
                for epoch in range(cfg.d_epoch):
                    ...
                    torch.save({'state_dict': leak_gan.dis.state_dict(),
                                'loss': leak_gan.dis_loss / data_gen.get_train_len()},
                               cfg.adv_dis_checkpoint_path + '\\Epoch{:0>3d}_adv_discriminator_loss{:.5f}.pth.tar'.format(
